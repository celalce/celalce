<table>
  <tr>
    <td width="20%">
      <img src="https://github.com/celalce.png" width="150" style="border-radius: 50%;">
    </td>
    <td width="80%">
      <h1>Hi there, I'm Celal Albayrak 👋</h1>
      <p>As an <b>IoT Developer & Full-Stack Programmer</b>, I specialize in bridging the gap between the physical world and digital systems through innovative automation.</p>
    </td>
  </tr>
</table>


## 🛠️ Skills & Tools
- **Languages:** PHP, JavaScript, Kotlin, HTML, CSS, C++
- **IoT & Hardware:** ESP32, ESP8266, MQTT Protocol, Home Assistant
- **Database:** MySQL, PDO Integration
- **Frameworks:** Laravel

## 🌐 Technologies
![PHP](https://img.shields.io/badge/php-%23777BB4.svg?style=for-the-badge&logo=php&logoColor=white)
![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=F7DF1E)
![Arduino](https://img.shields.io/badge/-Arduino-00979D?style=for-the-badge&logo=Arduino&logoColor=white)
![MySQL](https://img.shields.io/badge/mysql-%2300f.svg?style=for-the-badge&logo=mysql&logoColor=white)
![HTML5](https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white)
![MQTT](https://img.shields.io/badge/MQTT-3C5233?style=for-the-badge&logo=mqtt&logoColor=white)
![Google Home](https://img.shields.io/badge/Google%20Home-%234285F4.svg?style=for-the-badge&logo=google-home&logoColor=white)
![Home Assistant](https://img.shields.io/badge/home%20assistant-%2341BDF5.svg?style=for-the-badge&logo=home-assistant&logoColor=white)

## 🚀 Featured IoT Projects
- **Smart Home Automation:** Developed control systems for TV, doors, and lighting using ESP32 and MQTT.
- **Custom PHP Timer Interface:** Created server-side PHP scripts to trigger smart home automation scenarios.
- **Matching Game:** Developed and published a mobile application on the Google Play Store.

---
### 📫 Connect with me:
[LinkedIn](https://www.linkedin.com/in/celal-albayrak-b73554329/) | [Website](https://cabimox.com) | [E-Mail](mailto:celalce25@hotmail.com) [celalce25@hotmail.com]
